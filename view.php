...
$PAGE->set_url("{$CFG->wwwroot}/local/wb_faq/index.php", ['id' => $id]);
...
if ($id) {
    ...
    $entity = \local_wb_faq\settings_manager::get_settings($id);
...
// Rendere Func
$fs = get_file_storage();
$files = $fs->get_area_files($context->id, 'local_wb_faq', 'image', $id);
foreach ($files as $file) {
    $filename = $file->get_filename();
    if ($file->get_filesize() > 0) {
        $url = moodle_url::make_file_url('/pluginfile.php', '/1/local_wb_faq/image/' . $id . '/' . $filename);
    }
}
...
$entity->editurl = new moodle_url('/local/wb_faq/edit.php', array( 'id' => $id));
$entity->delurl = new moodle_url('/local/wb_faq/wb_faq.php', array( 'del' => $id , 'sesskey' => $USER->sesskey));
$entity->description = format_text($entity->description, FORMAT_HTML);



echo $OUTPUT->render_from_template('local_wb_faq/view', $entity);
